...
class BasicModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        where out_features[0] should have the shape:
            shape(-1, output_channels[0], 38, 38),
        """
        out_features = [self.bank1, self.bank2, self.bank3, self.bank4, self.bank5, self.bank6]
...
class BasicModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        for idx, feature in enumerate(out_features):
            feature_output = feature(feature_output)
            #print("index:   ", idx, "   feature:    ", feature_output.size())
            out_features[idx] = feature_output
        
        for idx, feature in enumerate(out_features):
            out_channel = self.output_channels[idx]
            feature_map_hight = feature.shape[3]
            feature_map_width = feature.shape[2]
            expected_shape = (out_channel, feature_map_width, feature_map_hight)
            assert feature.shape[1:] == expected_shape, \
                f"Expected shape: {expected_shape}, got: {feature.shape[1:]} at output IDX: {idx}"
        return tuple(out_features)
